...
def run(data: DataInput, h_function, start_time, time_limit) -> AlgorithmResult:
    ...
    while forward_open_queue.is_empty() is False and backward_open_queue.is_empty() is False:
        ...
        if current_forward_node.coordinates == current_backward_node.coordinates:
            ...
            goal_backward = backward_open_queue.remove()
            break
        ...
        if current_backward_node.coordinates in backward_visited:
            if current_backward_node.f_cost_of_path > backward_visited.get(current_backward_node.coordinates).f_cost_of_path:
                continue
            # print(f' duplicate: {current_backward_node.coordinates.x},{current_backward_node.coordinates.y} : '
            #       f'{current_backward_node.f_cost_of_path} // '
            #       f'{backward_visited.get(current_backward_node.coordinates).f_cost_of_path}')
