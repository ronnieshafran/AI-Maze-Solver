...
def dfs_contour(data: DataInput, node, f_limit, h_function, stats: StatsContainer, const_fix, start_time, runtime):
    next_f = sys.maxsize
    if time.process_time() - start_time >= runtime:
        return None, node.f_cost_of_path, stats, True

    if node.f_cost_of_path > f_limit:
        if stats.min_depth > node.depth:
            stats.min_depth = node.depth
        return None, node.f_cost_of_path, stats, False
    ...
    return None, next_f, stats, False
